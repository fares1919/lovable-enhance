console.log("[Background] Lovable Enhance service worker started");
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg && msg.action === "proxyFetch") {
    (async () => {
      try {
        // LOCAL MODE: detect lov.powerkits.net and lovable-api.com URLs and return mock data
        var urlStr = String(msg.url || "");
        var urlHost = "";
        try { urlHost = new URL(urlStr).hostname; } catch(e) {}
        var isBackendUrl = urlHost === "lov.powerkits.net" || urlHost.endsWith(".lov.powerkits.net")
          || urlHost === "lovable-api.com" || urlHost.endsWith(".lovable-api.com");

        if (isBackendUrl) {
          if (urlStr.indexOf("/validate-license") !== -1) {
            sendResponse({ ok: true, status: 200, data: {
              valid: true,
              session_id: "local_" + Date.now().toString(36),
          ...
          if (urlStr.indexOf("/notifications") !== -1) {
            sendResponse({ ok: true, status: 200, data: [] });
            return false;
          }
          if (urlStr.indexOf("/packages") !== -1) {
            sendResponse({ ok: true, status: 200, data: [] });
            return false;
          }
          if (urlStr.indexOf("/user_roles") !== -1) {
            sendResponse({ ok: true, status: 200, data: [] });
            return false;
          }
          if (urlStr.indexOf("/licenses") !== -1) {
            sendResponse({ ok: true, status: 200, data: [{ user_id: null }] });
            return false;
          }
          if (urlStr.indexOf("/extension_versions") !== -1) {
            sendResponse({ ok: true, status: 200, data: [] });
            return false;
          }
          if (urlStr.indexOf("/optimize-prompt") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, optimized_prompt: "" } });
            return false;
          }
          if (urlStr.indexOf("/create-lovable-project") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, projectId: null, error: "Use Lovable's web UI to create projects." } });
            return false;
          }
          if (urlStr.indexOf("/remove-watermark") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, message: "Use Lovable's built-in watermark removal in project settings." } });
            return false;
          }
          if (urlStr.indexOf("/publish-project") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, message: "Use Lovable's built-in publish feature." } });
            return false;
          }
          if (urlStr.indexOf("/enable-cloud") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, message: "Use Lovable's built-in cloud enable feature." } });
            return false;
          }
          if (urlStr.indexOf("/process-extension-payment") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true, transaction_id: null, message: "Payments are processed by Lovable directly." } });
            return false;
          }
          if (urlStr.indexOf("/feature_flags") !== -1) {
            sendResponse({ ok: true, status: 200, data: [] });
            return false;
          }
          if (urlStr.indexOf("/proxy-command") !== -1) {
            sendResponse({ ok: true, status: 200, data: { ok: true } });
            return false;
          }
          if (urlStr.indexOf("/storage/v1/") !== -1) {
            if (msg.method === "POST" || msg.method === "PUT") {
              sendResponse({ ok: true, status: 200, data: { Key: "local_" + Date.now().toString(36) } });
              return false;
            }
            sendResponse({ ok: true, status: 200, data: {} });
            return false;
          }
          // Catch-all for any remaining backend URL
          console.log("[Background] local-mode bypass for backend URL:", urlStr);
          sendResponse({ ok: true, status: 200, data: {} });
          return false;
        }

        // Non-backend URLs: proxy normally
  ...
  if (msg && msg.action === "downloadProject") {
    (async function () {
      try {
        // LOCAL MODE: use Lovable's own API to download source code
        const storage = await new Promise(r => chrome.storage.local.get(["lovable_token"], r));
        var resp = await fetch("https://api.lovable.dev/projects/" + msg.projectId + "/export", {
          method: "GET",
          headers: {
            "Authorization": "Bearer " + (msg.token || storage.lovable_token || ""),
