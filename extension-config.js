(function(){
  ...
  _f('EXTENSION_NAME', 'Lovable Enhance');
  _f('EXTENSION_VERSION', '2.0');
  _f('DEFAULT_LICENSE_USER_NAME', 'Lovable Enhance User');
...
function extensionFooterBadge() {
  var name = typeof EXTENSION_NAME !== "undefined" ? String(EXTENSION_NAME) : "Lovable Enhance";
...
function normalizeLicenseUserName(name) {
  ...
  if (!n || n.toLowerCase() === "test" || n.toLowerCase() === "user" || /gringow|powerkits/i.test(n)) {
    return typeof DEFAULT_LICENSE_USER_NAME !== 'undefined' ? DEFAULT_LICENSE_USER_NAME : "Lovable Enhance User";
  }
  return n;
}
